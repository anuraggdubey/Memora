...
import { RECENT_CHATS, USER } from "./workspace-data";
...
export default function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const filtered = RECENT_CHATS.filter((c) => c.title.toLowerCase().includes(search.toLowerCase()));
        ...
        <ul className="flex flex-col gap-0.5">
          {filtered.map((c) => (
            <li key={c.id}>
              ...
              >
                <span className="text-[13px] text-[#1A1A1A] truncate text-left">{c.title}</span>
